...
public class SmartPlugGrowLightTimerProvider extends ContentProvider {
	private static final String TAG = "SmartPlugGrowLightTimerProvider";
	...
	static {
		uriMatcher.addURI(
				SmartPlugContentDefine.SmartPlugGrowLightTimer.AUTHORITY,
				SmartPlugContentDefine.SmartPlugGrowLightTimer.ALL_RECORD,
				TIMER_ACTIONS);
		uriMatcher.addURI(
				SmartPlugContentDefine.SmartPlugGrowLightTimer.AUTHORITY,
				SmartPlugContentDefine.SmartPlugGrowLightTimer.ONE_RECORD
						+ "/#", TIMER_ACTION);
	}
	...
	public int delete(Uri uri, String where, String[] selectionArgs) {
		...
		switch (uriMatcher.match(uri)) {
			case TIMER_ACTIONS :
				count = gotaDB
						.delete(SmartPlugContentDefine.SmartPlugGrowLightTimer.TABLE_NAME,
								where, selectionArgs);
				break;
			case TIMER_ACTION :
				...
				count = gotaDB
						.delete(SmartPlugContentDefine.SmartPlugGrowLightTimer.TABLE_NAME,
								SmartPlugContentDefine.SmartPlugGrowLightTimer._ID
										+ "="
										+ carID
	...
	public Uri insert(Uri uri, ContentValues initialValues) {
		...
		switch (uriMatcher.match(uri)) {
			case TIMER_ACTIONS :
				tableName = SmartPlugContentDefine.SmartPlugGrowLightTimer.TABLE_NAME;
				break;
			default :
				break;
		}
		try {
			long rowId = gotaDB.insert(tableName,
					SmartPlugContentDefine.SmartPlugGrowLightTimer._ID, values);
			if (rowId > 0) {
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		switch (uriMatcher.match(uri)) {
			case TIMER_ACTIONS :
				cursor = gotaDB
						.query(true,
								SmartPlugContentDefine.SmartPlugGrowLightTimer.TABLE_NAME
										.toLowerCase(), projection, selection,
								selectionArgs, groupBy, null, sortOrder, null);
				break;
			case TIMER_ACTION :
				id = ContentUris.parseId(uri);
				where = SmartPlugContentDefine.SmartPlugGrowLightTimer._ID
						+ "=" + id;
				...
				if (TextUtils.isEmpty(sortOrder)) {
					orderBy = SmartPlugContentDefine.SmartPlugGrowLightTimer._ID;
				} else {
					orderBy = sortOrder;
				}

				cursor = gotaDB
						.query(SmartPlugContentDefine.SmartPlugGrowLightTimer.TABLE_NAME
	...
	public int update(Uri uri, ContentValues values, String where,
		...
		switch (uriMatcher.match(uri)) {
			case TIMER_ACTIONS :
				count = gotaDB
						.update(SmartPlugContentDefine.SmartPlugGrowLightTimer.TABLE_NAME,
