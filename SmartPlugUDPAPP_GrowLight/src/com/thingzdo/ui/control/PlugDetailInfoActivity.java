...
public class PlugDetailInfoActivity extends TitledActivity
		implements
			OnClickListener {
	...
	private void updateUpgradeAP(Intent intent) {
		...
		if (result == 0) {
			...
			tv_sb_content.setText(String.format("%s/%s (%.2f%%)", index_block,
					total_block, ((double) index_block * 100.0) / total_block));
	...
	private void init_IR_Data() {
		...
		list_smartplugtype.add("智能插座");
		list_smartplugtype.add("智能插座(新)");
		list_smartplugtype.add("智能窗帘");
		list_smartplugtype.add("智能水壶");
		list_smartplugtype.add("智能电池");
		list_smartplugtype.add("智能开关");
		list_smartplugtype.add("智能开关II");
		list_smartplugtype.add("智能电子秤");
		list_smartplugtype.add("智能小车");
		list_smartplugtype.add("智能植物生长灯");
		...
		sp_smartplugtype
				.setOnItemSelectedListener(new SPSmartPlugTypeListener());

		if (list_smartplugtype.size() > 0) {
			String typeName = PubFunc.getDeviceNameFromModule(
					mPlug.mSubDeviceType, mPlug.mSubProductType);
	...
	class SPSmartPlugTypeListener
		...
		public void onItemSelected(AdapterView<?> parent, View view,
			...
			String str_Current_NO = parent.getItemAtPosition(position)
					.toString();
	...
	private void InitView() {
		...
		if (null != plug) {
			...
			if ((plug.mSubDeviceType == PubDefine.DEVICE_SMART_PLUG && plug.mSubProductType == PubDefine.PRODUCT_PLUG)
					|| (plug.mSubDeviceType == PubDefine.DEVICE_SMART_PLUG_BOARD)) {
				// 1_1,
				// 2_
			...
			} else if (plug.mSubDeviceType == PubDefine.DEVICE_SMART_PLUG
					&& plug.mSubProductType == PubDefine.PRODUCT_SMART_PLUG_AIRCON) { // 1_2
				img_devicetype.setImageResource(R.drawable.smp_plug_small);
			} else if (plug.mSubDeviceType == PubDefine.DEVICE_SMART_PLUG
					&& plug.mSubProductType == PubDefine.PRODUCT_SMART_PLUG_ENERGE) { // 1_3
				img_devicetype.setImageResource(R.drawable.smp_plug_small);
			} else if (plug.mSubDeviceType == PubDefine.DEVICE_SMART_SWITCH
					&& plug.mSubProductType == PubDefine.PRODUCT_PLUG) { // 5_1
				img_devicetype.setImageResource(R.drawable.smp_plug_small);
			} else if (plug.mSubDeviceType == PubDefine.DEVICE_SMART_SWITCH
					&& plug.mSubProductType == PubDefine.PRODUCT_SMART_PLUG_AIRCON) { // 5_2
				img_devicetype.setImageResource(R.drawable.smp_plug_small);
			} else if (plug.mSubDeviceType == PubDefine.DEVICE_SMART_STEELYARD
					&& plug.mSubProductType == PubDefine.PRODUCT_PLUG) { // 10_1
				img_devicetype.setImageResource(R.drawable.smp_steelyard_small);
			} else if (plug.mSubDeviceType == PubDefine.DEVICE_SMART_CAR
					&& plug.mSubProductType == PubDefine.PRODUCT_PLUG) { // 11_1
				img_devicetype.setImageResource(R.drawable.smp_smartcar_small);
			} else if (plug.mSubDeviceType == PubDefine.DEVICE_SMART_GROWLIGHT
					&& plug.mSubProductType == PubDefine.PRODUCT_PLUG) { // 12_1
				img_devicetype.setImageResource(R.drawable.smp_growlight_small);
			} else {
				img_devicetype.setImageResource(R.drawable.smp_unknown_small);
			}
			...
			tv_moduletype.setText(PubFunc.getDeviceNameFromModule(
					mPlug.mSubDeviceType, mPlug.mSubProductType));
			...
			ll_smartplugtype = (LinearLayout) findViewById(R.id.ll_smartplugtype);
			if (true == PubDefine.RELEASE_VERSION) {
				ll_smartplugtype.setVisibility(View.GONE);
			} else {
	...
	private void Reset_to_AP() {
		...
		// mProgress = PubFunc.createProgressDialog(PlugDetailInfoActivity.this,
		// getString(R.string.str_reset_module), false);
		// mProgress.show();
		//
		// //0,APPPASSTHROUGH,smli123hz,6044712,15,0,0,BACK2AP,smli123hz,6044712#
		// String command_base = "APPBACK2AP," + PubStatus.g_CurUserName + "," +
		// mPlugId;
	...
	private void Upgrade_AP() {
		...
		String deviceType = PubFunc.getDeviceType(tv_type_select_content
				.getText().toString());
		...
		sb.append("APPUPGRADESTART")
				...
				.append(PubStatus.getUserName())
				.append(StringUtils.PACKAGE_RET_SPLIT_SYMBOL).append(mPlugId)
				.append(StringUtils.PACKAGE_RET_SPLIT_SYMBOL).append(auxFileNo)
				.append(StringUtils.PACKAGE_RET_SPLIT_SYMBOL)
