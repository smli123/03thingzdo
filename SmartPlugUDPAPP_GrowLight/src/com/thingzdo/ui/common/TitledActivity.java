...
public abstract class TitledActivity extends Activity {
	...
	protected void onCreateBaiduMap(Bundle savedInstanceState, int layoutResID,
			boolean backToExit) {
		mBack2Exit = backToExit;
		super.onCreate(savedInstanceState);
		requestWindowFeature(Window.FEATURE_NO_TITLE);
	...
	private void setButtonInfo(Button button, int textResId, int bgResId,
	...
	private void setButtonInfo(Button button, CharSequence text,
			Drawable background, OnClickListener onClick) {
		if (null != button) {
			button.setVisibility(View.VISIBLE);
			if (null != text) {
				button.setText(text);
	...
	/*
	 * protected void setButtonStatus() { Iterator iter =
	 * mView_Command.entrySet().iterator(); while (iter.hasNext()) { Map.Entry
	 * entry = (Map.Entry) iter.next(); Integer key = (Integer)entry.getKey();
	 * Integer val = (Integer)entry.getValue(); if (true ==
	 * CarGuardSmsSets.isCommandSend(key.intValue())){ Button btn =
	 * (Button)findViewById(val.intValue()); if (null != btn) {
	 * btn.setEnabled(false); PubFunc.startFlick(btn); } } } }
	 */
	...
	protected Handler timeoutHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			if (msg.what == AppServerReposeDefine.Socket_Connect_FAIL) {
				...
				// SmartPlugApplication.getContext().sendBroadcast(new
				// Intent(PubDefine.SOCKET_CONNECT_FAIL_BROADCAST));
				// String error = ret.mMessage;
				String error = SmartPlugApplication.getContext().getString(
						R.string.login_timeout);
				if (PubDefine.g_Connect_Mode != PubDefine.SmartPlug_Connect_Mode.Internet) {
					error = SmartPlugApplication.getContext().getString(
							R.string.oper_error);
				}
				...
				// this.removeCallbacks(timeoutProcess);
				String error = SmartPlugApplication.getContext().getString(
						R.string.login_cmd_socket_timeout_devices);
				PubFunc.thinzdoToast(SmartPlugApplication.getContext(), error);
	...
	protected void setTitleLeftImage(int bgResId, final OnClickListener onClick) {
		if (null != mLeftImage) {
			setImageInfo(mLeftImage, bgResId, onClick);
		}
	}

	protected void setTitleRightImage(int bgResId, final OnClickListener onClick) {
		if (null != mRightImage) {
			setImageInfo(mRightImage, bgResId, onClick);
		}
	}

	private void setImageInfo(ImageView image, int bgResId,
			OnClickListener onClick) {
		if (null != image) {
			...
			timeoutHandler
					.postDelayed(
							...
							PubDefine.g_Connect_Mode == PubDefine.SmartPlug_Connect_Mode.Internet
									? PubDefine.WAIT_SER_RESPONSE
									: PubDefine.WAIT_WIFI_RESPONSE);
		}
	};

	protected void sendMsgBin(boolean containCookie, byte[] msgBin,
			boolean needDelay) {
