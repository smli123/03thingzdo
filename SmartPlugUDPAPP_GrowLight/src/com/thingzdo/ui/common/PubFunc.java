...
public class PubFunc {
	...
	public static SmartPlugProgressDlg createProgressDialog(Context context,
			String message, boolean closeVisible) {
		SmartPlugProgressDlg progressDialog = SmartPlugProgressDlg
	...
	public static String getIPAddress() {
		...
		try {
			ReturnStr = java.net.InetAddress
					.getByName(PubDefine.THINGZDO_HOST_NAME);
	...
	public static String getTimerTypeLabel(Context context, int type) {
		switch (type) {
			case 0 :
				return SmartPlugApplication.getContext().getString(
						R.string.power);
			case 1 :
				return SmartPlugApplication.getContext().getString(
						R.string.nightlight);
			case 2 :
				return SmartPlugApplication.getContext().getString(
						R.string.bell);
			case 3 :
				return SmartPlugApplication.getContext()
	...
	public static String getAppVersion() {
		...
		try {
			info = manager.getPackageInfo(SmartPlugApplication.getContext()
					.getPackageName(), 0);
	...
	// return "智能开关";
	// } else if (plugType.equals("5_2") == true) {
	// return "智能开关II";
	...
	public static String getDeviceType(String plugType) {
		if (plugType.equals("未知类型") == true) {
			...
			return "5_1";
		} else if (plugType.equals("智能开关II") == true) {
			return "5_2";
			...
			return "11_1";
		} else if (plugType.equals("智能植物生长灯") == true) {
			return "12_1";
		} else {
			return "不可能的类型";
		}
	...
	public static String getDeviceNameFromModule(String plugType,
		...
		if (plugType.equals(PubDefine.DEVICE_UNKNOWN) == true) {
			return "未知类型";
				...
				&& ((deviceType.equals(PubDefine.PRODUCT_PLUG) == true))) { // 1_1
			return "智能插座";
		} else if ((plugType.equals(PubDefine.DEVICE_SMART_PLUG) == true)
				&& ((deviceType.equals(PubDefine.PRODUCT_SMART_PLUG_AIRCON) == true))) { // 1_2
			return "智能插座(新)";
		} else if ((plugType.equals(PubDefine.DEVICE_SMART_CURTAIN) == true)
				&& ((deviceType.equals(PubDefine.PRODUCT_PLUG) == true))) { // 3_1
			return "智能窗帘";
		} else if ((plugType.equals(PubDefine.DEVICE_SMART_WINDOW) == true)
				&& ((deviceType.equals(PubDefine.PRODUCT_PLUG) == true))) { // 4_1
			return "智能窗口";
		} else if ((plugType.equals(PubDefine.DEVICE_SMART_SWITCH) == true) // 5_1
				&& (deviceType.equals(PubDefine.PRODUCT_PLUG) == true)) {
			return "智能开关";
		} else if ((plugType.equals(PubDefine.DEVICE_SMART_SWITCH) == true)
				&& (deviceType.equals(PubDefine.PRODUCT_SMART_PLUG_AIRCON) == true)) { // 5_2
			return "智能开关II";
		} else if (plugType.equals(PubDefine.DEVICE_SMART_KETTLE) == true) { // 6_1
			return "智能水壶";
		} else if (plugType.equals(PubDefine.DEVICE_SMART_SIMULATION_PC) == true) { // 7_1
			return "电脑模拟软件";
		} else if (plugType.equals(PubDefine.DEVICE_SMART_BATTERY) == true) { // 8_1
			return "智能电池";
		} else if (plugType.equals(PubDefine.DEVICE_SMART_AIRCONSTUDY) == true) { // 9_1`
			return "智能红外学习";
		} else if (plugType.equals(PubDefine.DEVICE_SMART_STEELYARD) == true) { // 10_1
			return "智能电子秤";
		} else if (plugType.equals(PubDefine.DEVICE_SMART_CAR) == true) { // 11_1
			return "智能小车";
		} else if ((plugType.equals(PubDefine.DEVICE_SMART_GROWLIGHT) == true)
				&& ((deviceType.equals(PubDefine.PRODUCT_PLUG) == true))) { // 12_1
			return "智能植物生长灯";
		} else {
			return "不可能的类型";
		}
	...
	private static Map<String, String> getDeviceMap() {
		...
		m_mapTypes.put("10", PubDefine.DEVICE_SMART_STEELYARD);
		m_mapTypes.put("11", PubDefine.DEVICE_SMART_CAR);
		m_mapTypes.put("12", PubDefine.DEVICE_SMART_GROWLIGHT);
