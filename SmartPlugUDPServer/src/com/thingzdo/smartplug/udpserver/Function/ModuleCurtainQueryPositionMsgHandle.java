...
public class ModuleCurtainQueryPositionMsgHandle implements ICallFunction{
	/**********************************************************************************************************
	...
	public int call(Runnable thread_base, String strMsg) 
	{
		...
		String strReserved			= strRet[4].trim();	// 预留位
		...
		try
		{
			...
			try {	
				/* 待模块返回 */
				String moduleMsg = strCookie + "," + ServerCommDefine.CURTAIN_QUERYPOSITION_MSG_HEADER + "," + strUserName + "," + strModuleId + "," + strReserved + "#";
				iRet = NotifyToModule(moduleMsg);
				if (ServerRetCodeMgr.SUCCESS_CODE != iRet)
				{
					return iRet;
				}
	...
	public int resp(Runnable thread_base, String strMsg) {
		...
		try
		{
			...
			//给APP回复成功
			ResponseToAPP(ServerCommDefine.APP_CURTAIN_QUERYPOSITION_MSG_HEADER, strUserName, strModuleID, ServerRetCodeMgr.SUCCESS_CODE, strStatus);
			return ServerRetCodeMgr.SUCCESS_CODE;
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
